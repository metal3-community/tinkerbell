...
func setupMacvlan(log logr.Logger, sourceIface, ifaceName string) (func(), error) {
	...
	// IFA_F_NOPREFIXROUTE suppresses the 255.255.255.255/32 RTN_BROADCAST
	// entry that the kernel would otherwise add to the local routing table.
	// Without it, fib_lookup for 255.255.255.255 constrained to this interface
	// (via SO_BINDTODEVICE) returns nothing and sendmsg fails with EINVAL.
	// Add it explicitly so broadcast replies from the DHCP server succeed while
	// keeping the pod routing table free of a 169.254.0.0/16 prefix route.
	bcastRoute := &netlink.Route{
		LinkIndex: podLink.Attrs().Index,
		Dst:       &net.IPNet{IP: net.IPv4bcast, Mask: net.CIDRMask(32, 32)},
		Type:      unix.RTN_BROADCAST,
		Scope:     netlink.SCOPE_LINK,
		Protocol:  unix.RTPROT_KERNEL,
		Table:     unix.RT_TABLE_LOCAL,
	}
	if err := netlink.RouteAdd(bcastRoute); err != nil {
		return nil, fmt.Errorf("add broadcast route to macvlan %q: %w", ifaceName, err)
	}
