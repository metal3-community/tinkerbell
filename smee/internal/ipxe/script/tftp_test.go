...
func TestHandleTFTP_ErrorMessages(t *testing.T) {
	...
	// Requests this handler doesn't own return ErrNotFound so the router
	// falls through to the next route (eg. on-disk asset serving).
	tests := []struct {
		name     string
		filename string
	}{
		...
		{
			name:     "non-MAC token with valid format",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !errors.Is(err, handler.ErrNotFound) {
				t.Errorf("expected ErrNotFound for filename %s, got: %v", tt.filename, err)
			}
		})
	}
}
