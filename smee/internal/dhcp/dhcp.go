...
// IsRaspberryPI5 checks if the mac address is from a Raspberry Pi 5 by matching against the
// OUI registration used for Pi 5 production. Raspberry Pi reuses OUIs across models, so this is
// a best-effort match: 88:A2:9E is the newest registration and the one most specific to the Pi 5.
// A match here implies IsRaspberryPI is also true.
func IsRaspberryPI5(mac net.HardwareAddr) bool {
	return bytes.HasPrefix(mac, []byte{0x88, 0xa2, 0x9e}) // 88:A2:9E
}
