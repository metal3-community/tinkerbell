...
func (h Handler) HandleTFTP(filename string, rf io.ReaderFrom) error {
	...
	// Return ErrNotFound for unmatched shapes so this handler composes as a
	// fall-through route (eg. behind the CRD-backed pxelinux/RPi routes and
	// ahead of the on-disk asset routes).
	if macStr == "" {
		return fmt.Errorf("filename does not match expected patterns %q: %w", filename, handler.ErrNotFound)
	}

	log := h.Logger.WithValues("event", "pxelinux", "filename", filename, "macStr", macStr)

	// Parse MAC address. A non-MAC token (eg. a Raspberry Pi serial number
	// prefix) is not an error: fall through so an asset route can serve it.
	mac, err := net.ParseMAC(macStr)
	if err != nil {
		log.Info("token is not a MAC address; passing on request", "error", err)
		return fmt.Errorf("no MAC in %q: %w", filename, handler.ErrNotFound)
	}
